...
func download_and_save(url string, save_path string, ch chan<- string) {
    ...
    save_file, err := os.OpenFile(save_path, os.O_RDWR|os.O_CREATE|os.O_TRUNC,
                                0666)
    if err != nil {
    ...
    written_bytes, err := io.Copy(save_file, response.Body)
    response.Body.Close()
    if err != nil {
...
func main() {
    ...
    for _, r := range resourceLocations {
        go download_and_save(urlPrefix+r, savePath+r, ch)
    }

...
.DESCRIPTION
    ...
    Pipe-safe: the whole body runs inside a scriptblock (no variables leak
    into your session under "irm ... | iex") and never calls "exit" (which
    would end YOUR shell). Failures throw; early paths return.
...
& {
    ...
    try {
        if ($FromDir -ne "") {
            ...
            # Fetch the checksum separately from the comparison: a missing
            # manifest degrades to a warning, but a MISMATCH must fail hard.
            $want = $null
            try {
